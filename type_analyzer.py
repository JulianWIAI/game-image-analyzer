...
- Load your own type data via CSV/JSON
- Optional: Fetch Pokemon data from PokeAPI (for Pokemon projects only)

CSV Format for custom type data:
- Required: 'name' column (must match scan results)
- Required: 'type_primary' column (e.g., 'fire', 'hero', 'enemy', 'friendly')
- Optional: 'type_secondary', 'category', 'attribute', etc.
"""
...
class TypeAnalyzer:
    def __init__(self, scan_csv_path, type_data_path=None, output_dir="output/analysis", 
        ...
        # Project name for chart titles
        if project_name:
            self.project_name = project_name
        else:
            self.project_name = "Character"  # Generic default
    ...
    def load_type_data(self, path):
        ...
        Expected format:
        - CSV: name,type_primary,type_secondary,... (any additional columns)
        - JSON: [{"name": "...", "type_primary": "...", ...}, ...]
        """
        path = Path(path)
    ...
    def fetch_pokemon_types(self, pokemon_names=None, save_path=None):
        """
        [POKEMON ONLY] Fetch Pokemon type data from PokeAPI.
        This is a convenience method for Pokemon projects.
        For other games, use load_type_data() with your own CSV/JSON.
        ...
        print(f"Fetching Pokemon data for {len(pokemon_names)} entries...")
        print("(This only works for Pokemon - for other games, create your own type CSV)")
        
        self.project_name = "Pokemon"  # Set project name
        type_data = []
    ...
    def merge_data(self):
        ...
        if self.type_df is None:
            ...
            print("  2. Use fetch_pokemon_types() for Pokemon projects")
            return None
    ...
    def create_type_color_heatmap_static(self, merged_df=None, top_colors=15):
        ...
        ax.set_title(f"Color Distribution by {self.project_name} Type", fontsize=14, fontweight='bold')
    ...
    def create_type_shape_heatmap_static(self, merged_df=None):
        ...
        ax.set_title(f"Shape Distribution by {self.project_name} Type", fontsize=14, fontweight='bold')
    ...
    def create_type_analysis_interactive(self, merged_df=None):
        ...
        fig.update_layout(
            title=dict(text=f"{self.project_name} Type Visual Analysis Dashboard", font=dict(size=20)),
            height=900,
            showlegend=False,
    ...
    def create_attribute_correlation_chart(self, merged_df=None):
        ...
        # Plot 1: Color vs some numeric attribute (happiness or custom)
        ax1 = axes[0, 0]
        if has_happiness:
            ...
            if avg_happiness:
                ...
                hex_colors = [self._get_color_hex(c) for c in colors_list]
                ax1.barh(range(len(colors_list)), values, color=hex_colors, edgecolor='black')
            ...
            ax1.text(0.5, 0.5, "No happiness data\n(Pokemon-specific)", ha='center', va='center', transform=ax1.transAxes)
            ax1.set_title("Color vs Attribute", fontweight='bold')
        
        # Plot 2: Shape vs attack or other stat
        ax2 = axes[0, 1]
        if has_attack:
            ...
            if avg_attack:
                ...
                colors_map = plt.cm.Reds(np.linspace(0.3, 0.9, len(shapes)))
                ax2.bar(range(len(shapes)), values, color=colors_map, edgecolor='black')
            ...
            ax2.text(0.5, 0.5, "No attack data\n(Add 'attack' column to type CSV)", ha='center', va='center', transform=ax2.transAxes)
            ax2.set_title("Shape vs Attribute", fontweight='bold')
        
        # Plot 3: Category comparison (legendary vs regular, or custom categories)
        ax3 = axes[1, 0]
        if has_legendary:
            ...
            ax3.text(0.5, 0.5, "No category data\n(Add 'is_legendary' or similar)", ha='center', va='center', transform=ax3.transAxes)
            ax3.set_title("Category Comparison", fontweight='bold')
        
        # Plot 4: Type stats scatter
        ax4 = axes[1, 1]
        if has_attack and has_defense:
            ...
            ax4.text(0.5, 0.5, "No attack/defense data\n(Add to type CSV)", ha='center', va='center', transform=ax4.transAxes)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze type-visual correlations for any game or image collection",
        epilog="""
Examples:
  # With custom type data (any game):
  python type_analyzer.py scan_results.csv -t my_types.csv -n "Mario"
  
  # With Pokemon API:
  python type_analyzer.py scan_results.csv --fetch-pokemon
...
Type CSV format:
  ...
  Bowser,villain,fire,aggressive
    ...
    parser.add_argument("-n", "--name", default="Character", help="Project name (e.g., 'Pokemon', 'Mario', 'Zelda')")
    parser.add_argument("--fetch-pokemon", action="store_true", help="[Pokemon only] Fetch type data from PokeAPI")
    ...
    if analyzer.type_df is not None:
        ...
        print("Options:")
        print("  1. Provide type data with: -t your_types.csv")
        print("  2. For Pokemon: --fetch-pokemon")
        print("\nSee --help for CSV format examples.")
